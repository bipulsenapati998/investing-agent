...
async def create_and_message(worker, creator_id, i: int):
    try:
        ...
        logger.error(
            f"[app.py]: Failed to run worker {i} due to exception: {e}", exc_info=True
        )


async def main(no_of_agents: int = 5):
    agent_folder = "agent"
    ...
    logger.info(f"[app.py]: Creating {no_of_agents} agents and generating ideas")
    routines = [
        create_and_message(worker, creator_id, i) for i in range(1, no_of_agents + 1)
    ]
...
if __name__ == "__main__":
    ...
    asyncio.run(main(config.no_of_agents))
